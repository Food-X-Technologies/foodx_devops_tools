...
class SingularSubscription(pydantic.BaseModel):
    """Definition of a singular subscription."""
...
ValueType = typing.Dict[str, SingularSubscription]


class SubscriptionsDefinition(pydantic.BaseModel):
    """Definition of deployment subscriptions."""
    ...
    def check_subscriptions(
        ...
        if not value:
            raise ValueError("Empty subscription names prohibited")
...
def load_subscriptions(file_path: pathlib.Path) -> SubscriptionsDefinition:
    ...
    Raises:
        SubscriptionsDefinitionError: If an error occurs loading the file.
    """
